...
export function CommentSection({ reviewId, initialCount }: { reviewId: string; initialCount: number }) {
  ...
  return (
    <div className="font-punk-mono">
      <button type="button" onClick={handleToggle} className="text-[11px] text-ink-600 hover:text-ink">
        💬 {count}
      </button>
      ...
      {expanded && (
        <div className="mt-2.5 flex flex-col gap-2.5">
          ...
          {comments && comments.length === 0 && <p className="text-[11px] text-ink-500 m-0">No comments yet.</p>}
